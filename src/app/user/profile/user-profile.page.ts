...
export class UserProfilePage implements OnInit {
  ...
  private getOffers(): void {
    this.categoryService.getCategories().subscribe(result => {
      ...
      console.log("numCategories = " + this.categories.length);

      for(var i:number = 0;i<this.categories.length;i++) {
        console.log("category[" + i + "].ImageName = " + this.categories[i].ImageName);
      }

      this.offerService.getOffersOfUser(this.userID)
      .subscribe(result => {
          this.offers = result; 

          for(var i:number=0;i<this.offers.length;i++) {
            for(var c:number = 0;c<this.categories.length;c++) {
              if(this.offers[i].CategoryID == this.categories[c].CategoryID) {
                this.offers[i].CategoryImageFullLazy = "./assets/interests/" + this.categories[c].ImageName;
                break;
              }
            }
          }
  ...
  onMakeOffer(): void {
    ...
    this.navCtrl.navigateForward("/app/offer")
  }

  onOfferPage(offerID:string): void {
    console.log("Go to offer page : " + offerID);
    let navigationExtras: NavigationExtras = { state: { offerID: offerID } };
    this.navCtrl.navigateForward("/app/offer-details", navigationExtras)
  }
}
